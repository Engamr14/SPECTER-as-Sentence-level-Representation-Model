...
#from triplet_sampling_parallel import triplet_sampling_parallel
...
class TrainingInstanceGenerator:
    ...
    def __init__(self,
        ...
        self.data = data
        with open('samples.pickle', 'rb') as my_file:
            self.samples = pickle.load(my_file)
    ...
    def get_raw_instances(self, query_ids, subset_name=None, n_jobs=10):
        ...
        # instances = []
        tsp = triplet_sampling_parallel(list(self.metadata.keys()), self.data, self.margin_fraction, self.samples_per_query, self.ratio_hard_negatives, query_ids, self.samples)
        for triplet in tsp.generate_triplets(data_subset=subset_name, n_jobs=n_jobs):
            #print('\ntriplet:\n', triplet)
            #break
...
def get_instances(data, query_ids_file, metadata, data_source=None, n_jobs=1, n_jobs_raw=12,
    ...
    #print(f'\nThis is the results of get_raw_inctances that generted in the function get_instances:\n\
    #     the size of the generated list is: {len(instances_raw)}\n\
    ...
    if n_jobs == 1:
        ...
        with Pool(n_jobs) as p:
            instances = list(tqdm.tqdm(p.imap(get_instance, instances_raw)))
...
def main(data_files, train_ids, val_ids, test_ids, metadata_file, outdir, n_jobs=1, njobs_raw=1,
    ...
    for data_file, train_set, val_set, test_set in zip(data_files, train_ids, val_ids, test_ids):
        ...
        metrics = {}
        for ds_name, ds in zip(('val', 'train', 'test'), (val_set, train_set, test_set)):
            print(f'\n  ####### Start {ds_name} processing #######\n')
...
if __name__ == '__main__':
    ...
    args['bert_vocab'] = 'data/scibert_scivocab_uncased/vocab.txt'
    #ap.add_argument('--concat-title-abstract', action='store_true', default=False)
    args['concat_title_abstract'] = False
    ...
    args['included_text_fields'] = 'title abstract'
    #args = ap.parse_args()
    args = DotMap(args)
