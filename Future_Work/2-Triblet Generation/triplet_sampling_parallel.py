...
class triplet_sampling_parallel:
    def __init__(self, paper_ids, coviews, margin_fraction, samples_per_query, ratio_hard_negatives, query_ids, samples):
        self._coviews = coviews
        self._margin_fraction = margin_fraction
        self._samples_per_query = samples_per_query
        self._ratio_hard_negatives = ratio_hard_negatives
        self._query_ids = query_ids
        self._paper_ids_set = set(paper_ids)
    ...
    def _get_triplet(self, query):
        #print('\nSECOND\n') #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if query not in self.samples:
            print('\n* query is not in the samples provided *\n')
            return []
        '''
        # self.coviews[query] is a dictionary of format {paper_id: {count: 1, frac: 1}}
        candidates = [(k, v['count']) for k, v in self._coviews[query].items()]
        #print('\nquery: ', query)
        #print('candidates\n', candidates)
        #print('done')
        ...
        if len(candidates) > 1:
            ...
            coview_spread = 0
        margin = self._margin_fraction * coview_spread  # minimum margin of coviews between good and bad sample
        ...
        if len(candidates) > 2 and margin != 0:
            ...
            if neg_len > 0:
                ...
                n_hard_samples = math.ceil(self._ratio_hard_negatives * self._samples_per_query)
            ...
            #print('The hard negative samples are in the form of [query, pos, hard_neg]:\n', results)
    
            n_easy_samples = self._samples_per_query - len(results)
    
            # ---------- easy triplets
    
            # valid candidates are those with zeros
            candidates_zero = list(self._paper_ids_set.difference([i[0] for i in candidates] + [query]))
            ...
            if candidates and len(candidates_pos) > 0:
                ...
                #print('The easy negative samples are in the form of [query, pos, easy_neg]:\n', easy_samples)
                results.extend(easy_samples)
        '''
        results = self.samples[query]
        return results
    
    
    def generate_triplets(self, data_subset=None, n_jobs=4): # n_jobs were = 1
        print('FIRST') #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        
        logger.info(f'generating triplets with: samples_per_query:{self._samples_per_query},'
                    f'ratio_hard_negatives:{self._ratio_hard_negatives}, margin_fraction:{self._margin_fraction}')
        #if n_jobs == 1:
        results = []
        for query in tqdm.tqdm(self._query_ids):
            result = self._get_triplet(query)
            if result == []:
                continue
            else:
        ...
        '''
            ...
            with Pool(n_jobs) as p:
                ...
                results = list(tqdm.tqdm(p.imap(self._get_triplet, self._query_ids), total=len(self._query_ids)))
                #print(f'\n**_________________**\n{results}\n**________________**\n')
        else:
            raise RuntimeError(f"bad argument `n_jobs`={n_jobs}, `n_jobs` should be -1 or >0")
        
        with open('my_results.pickle', 'wb') as output:
